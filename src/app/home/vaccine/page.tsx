...
const VaccineManagement = () => {
  ...
  const getLastName = (fullName) => {
    if (!fullName) return "";
    // Split the name by spaces and take the last word as the last name
  ...
  const filteredData = useMemo(() => {
    ...
    // Sort the filtered data by mother's last name
    return filtered.sort((a, b) => {
      const lastNameA = getLastName(a.mothers_name);
      const lastNameB = getLastName(b.mothers_name);
  ...
  const downloadExcel = () => {
    ...
    const excelData = filteredData.map((infant) => {
      ...
      return {
        ...
        Height: `${infant.height} cm`,
        Baranggay: infant.address?.baranggay || "",
        // Add a hidden column for last name sorting (won't be visible in Excel)
        _lastNameSort: getLastName(infant.mothers_name),
      };
    });

    // Sort by mother's last name
    excelData.sort((a, b) =>
      a["_lastNameSort"].localeCompare(b["_lastNameSort"])
    );

    // Remove the hidden sort column before export
    excelData.forEach((row) => delete row["_lastNameSort"]);
      ...
      <main className="p-4 md:p-8 font-[family-name:var(--font-geist-sans)] flex flex-col gap-8 overflow-x-auto">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="bg-[#f4faff] p-4 rounded-md border border-[#dbedff]">
              <p className="text-sm text-[#333]">Sorting</p>
              <p className="text-lg font-bold text-[#8993ff] truncate">
